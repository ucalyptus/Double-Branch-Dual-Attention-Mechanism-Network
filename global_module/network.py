...
class CDCNN_gabor(nn.Module):
    def __init__(self, band, classes):
        ...
        self.conv2 = GaborConv2d(in_channels=384, out_channels=128, kernel_size=(1, 1), device=device)
        self.res_net1 = Residual_2D(128, 128, (1, 1), (0, 0), batch_normal=True)
        self.res_net2 = Residual_2D(128, 128, (1, 1), (0, 0))

        self.conv3 = nn.Sequential(
            nn.ReLU(),
            GaborConv2d(in_channels=128, out_channels=128, kernel_size=(1, 1), device=device)
        )
        self.conv4 = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(0.5),
...
class GaborNN(nn.Module):
    ...
    def forward(self, x):
        ...
        print(x.shape)
        x = nn.MaxPool2d((7,7))(x)
        print(x.shape)
        x = F.leaky_relu(self.c1(x))
        print(x.shape)
        x = nn.MaxPool2d((5,5))(x)
        print(x.shape)
        x = x.view(-1, 384*3*3)
        print(x.shape)
